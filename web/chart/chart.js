...
//without delta, getDataBounds stretches the data to fit into the chart and can lie visually as to what is the closest neighbor
// just creates a lot of deadspace and stretching is default for the chart 
   #getDataBounds() {
      const { samples } = this;
   ...
   #draw() {
      ...
      //when sketchpad is open, will show the current drawings on the chart
      if (this.dynamicPoint) {
         ...
         //fade out the background of the chart to visualize the dynamic moving point
         graphics.drawPoint(ctx, pixelLoc, "rgba(255, 255, 255, 0.7", 1000000000000)
         //move the chart and draw a line from the dynamicPoint to it's nearest neighbor
         ctx.beginPath()
         ctx.moveTo(...pixelLoc)
         ...
         //draws dynamic point as the predicted image based off the predicted label on the chart
   ...
   #drawSamples(samples) {
      ...
      for (const sample of samples) {
         ...
         const style = this.styles[label] || this.styles.default;
         if (!style) {
            console.warn(`No style defined for label: ${label}`);
            continue;
         }
